...
uint8_t hd_camera_protocol_decode(
    ...
    *cmd_out = recv_data_in[3];
    ...
    if (length != (recv_data_size_in - 10)) {
        printf("length=%d,payload长度=%d 整个长度-10 != payload长度\n", length, recv_data_size_in);
        return 4; // 数据长度超过实际数据
    }
...
int hd_camera_protocol_parse_pic_info(const char *file_name, hd_parse_pic_infos *infos) {
    ...
    // 5. trigger angel / type
    unsigned long type = strtoul(parts[6], NULL, 10);
    infos->trigger_type = (uint8_t) type;
    unsigned long angel = strtoul(parts[5], NULL, 10);
    infos->trigger_angel = (uint8_t) angel;
