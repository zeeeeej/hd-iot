...
import com.yunext.iot.platform.selectSystemFile
    ...
    LaunchedEffect(address, fileVO) {
        launch {
            val todo = withContext(Dispatchers.IO) {
                ...
                val result = HDRequest.SystemOTA.encode(
                    HDRequest.SystemOTA(tmp.fileSize, tmp.fileMd5)
                )
                when (result) {
                    is HDResult.Fail -> {
                    ...
                    is HDResult.Success<ByteArray> -> {
                        HDProtocolFrame(
                            address = address,
                            cmd =
                                HDProtocolCmd.SystemOTANotice.key, result.data
                        )
                    }
                }
            }
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            Text("选择文件",Modifier.clickable {
                coroutineScope.launch {
                    val file = selectSystemFile()?:return@launch
                    fileVO = file
                }
            })
...
//        Text("${frame}")
        ...
        var snapResponse: HDResponse.SystemOTANotice? by remember {
            mutableStateOf(null)
        }
            ...
            launch {
                try {
                    val resp = withContext(Dispatchers.IO) {
                        val result = HDResponse.SystemOTANotice.decode(output.payload.byteArray)
                        when (result) {
                            is HDResult.Fail -> null
                            is HDResult.Success<HDResponse.SystemOTANotice> -> result.data
                        }
                    }
                    snapResponse = resp
                } catch (e: Exception) {
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            Text("升级系统文件${if (otaFileSize>0)"$otaFileSize bytes sending ..." else ""}", modifier = Modifier.clickable {
                coroutineScope.launch {
