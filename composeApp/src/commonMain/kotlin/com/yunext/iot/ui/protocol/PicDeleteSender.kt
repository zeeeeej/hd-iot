...
fun PicDeleteSender(
    ...
    LaunchedEffect(address, picId) {
        launch {
            if (picId==0xff.toUByte()){
                inputFrame = HDProtocolFrame(
                    address = address,
                    cmd =
                        cmd.key, byteArrayOf(picId.toByte())
                )
                return@launch
            }
            ...
            val todo = withContext(Dispatchers.IO) {
                ...
                val result = HDRequest.PicDelete.encode(
                    HDRequest.PicDelete(info.id)
                )
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            CommonSender(modifier = Modifier, frame = inputFrame, onSend = {
                ...
                Row(
                    ...
                    Text("删除所有",modifier = Modifier.clickable {
                        picId = 0xff.toUByte()
                    })
...
//        Text("${frame}")
        ...
        var snapResponse: HDResponse.PicDelete? by remember {
            mutableStateOf(null)
        }
        ...
        LaunchedEffect(output) {
            launch {
                val resp = withContext(Dispatchers.IO) {
                    val result = HDResponse.PicDelete.decode(output.payload.byteArray)
                    when (result) {
                        is HDResult.Fail -> null
                        is HDResult.Success<HDResponse.PicDelete> -> result.data
                    }
                }
                snapResponse = resp
            }
